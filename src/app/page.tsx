...
export default function Home() {
  ...
  const getTransactionAnalytics = () => {
    ...
    if (trxDashPreset === 'today') {
      list = list.filter(t => t.tanggal === todayStr);
    } else if (trxDashPreset === '7days') {
      const d7 = new Date();
      d7.setDate(d7.getDate() - 7);
      const monthStr = String(d7.getMonth() + 1).padStart(2, '0');
      const dayStr = String(d7.getDate()).padStart(2, '0');
      const d7Str = `${d7.getFullYear()}-${monthStr}-${dayStr}`;
      list = list.filter(t => t.tanggal >= d7Str);
    } else if (trxDashPreset === 'month') {
      const currentMonthStr = todayStr.substring(0, 7);
      list = list.filter(t => t.tanggal.startsWith(currentMonthStr));
    }

    if (filterDtlStart) {
      list = list.filter(t => t.tanggal >= filterDtlStart);
    }
    if (filterDtlEnd) {
      list = list.filter(t => t.tanggal <= filterDtlEnd);
    }
    if (filterDtlCategory) {
      list = list.filter(t => String(t.kategori_id) === filterDtlCategory);
    }
    ...
    const catMap: Record<string, { name: string; type: 'pemasukan' | 'pengeluaran'; total: number; count: number }> = {};
    const userMap: Record<string, { username: string; totalPemasukan: number; totalPengeluaran: number; count: number }> = {};

    list.forEach(t => {
      const nom = Number(t.nominal) || 0;
      const isPem = t.kategori?.tipe === 'pemasukan';
      const catName = t.kategori?.nama_kategori || `Kategori ID ${t.kategori_id}`;
      const username = t.user?.username || `User ID ${t.created_user_id || '-'}`;
      ...
      if (!catMap[catName]) {
        catMap[catName] = { name: catName, type: isPem ? 'pemasukan' : 'pengeluaran', total: 0, count: 0 };
      }
      catMap[catName].total += nom;
      catMap[catName].count++;

      if (!userMap[username]) {
        userMap[username] = { username, totalPemasukan: 0, totalPengeluaran: 0, count: 0 };
      }
      if (isPem) {
        userMap[username].totalPemasukan += nom;
      } else {
        userMap[username].totalPengeluaran += nom;
      }
      userMap[username].count++;
    });

    const netProfit = totalPemasukan - totalPengeluaran;
    const totalCount = countPemasukan + countPengeluaran;
    const avgTicket = totalCount > 0 ? (totalPemasukan + totalPengeluaran) / totalCount : 0;
    const turnover = totalPemasukan + totalPengeluaran;
    const incomeRatio = turnover > 0 ? (totalPemasukan / turnover) * 100 : 0;

    const topIncomeCategories = Object.values(catMap)
      .filter(c => c.type === 'pemasukan')
      .sort((a, b) => b.total - a.total);

    const topExpenseCategories = Object.values(catMap)
      .filter(c => c.type === 'pengeluaran')
      .sort((a, b) => b.total - a.total);

    const userAnalyticsList = Object.values(userMap).sort((a, b) => (b.totalPemasukan + b.totalPengeluaran) - (a.totalPemasukan + a.totalPengeluaran));
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-[#0d1222] border border-slate-800 rounded-xl pl-4 pr-11 py-3 text-sm text-white focus:outline-none focus:border-emerald-500/40 focus:ring-2 focus:ring-emerald-500/10 transition-all font-medium"
                    />
                    <button
                      type="button"
            ...
            <main className="max-w-6xl mx-auto px-6 mt-8">
              ...
              {activeTab === 'transaction_dtl' && currentUser?.role === 'owner' && (
                <div className="space-y-8 animate-in fade-in duration-200">
                  ...
                  {/* 2. DASHBOARD ANALISIS TRANSAKSI WIDGET */}
                  {(() => {
                  ...
                  {/* 3. SEKSYEN BAWAH: DAFTAR DATA & FILTER */}
                          ...
                          <div>
                            ...
                            <div className="relative">
                              <input
                                ...
                                className="w-full bg-[#0d1222] border border-slate-800 rounded-xl pl-4 pr-11 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-500/40 focus:ring-2 focus:ring-emerald-500/10 transition-all font-medium"
                              />
                              <button
                                type="button"
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-[#0d1222] border border-slate-800 rounded-xl pl-3 pr-10 py-2.5 text-xs text-white focus:outline-none focus:border-emerald-500/40"
                    />
                    <button
                      type="button"
